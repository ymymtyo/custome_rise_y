...
<main>
 ...
 <!-- 登録フォーム -->
  ...
  <table border="1" >
   ...
   <tbody>
     <c:forEach var="customerSearch" items="${sessionScope.customerSearch}">
			 <tr>
			    ...
			    <c:url var="delete" value="/CustomerDeleteServlet">
			 	 <c:param name="customer_id" value="${customerSearch.getSearch_customer_id()}"/>
			 	</c:url>
			    ...
			    <td><c:out value="${customerSearch.getSearch_customer_id()}" /></td>
			 	<td><c:out value="${customerSearch.getSearch_customer_name()}" /></td>	 	
			 </tr>
   	 </c:forEach>
