     ...
     <c:forEach var="goods" items="${sessionScope.goods}">
			 <tr>
			    ...
			    <c:url var="delete" value="/GoodsDeleteServlet">
			 	 <c:param name="goods_id" value="${goods.getGoods_id()}"/>
			 	</c:url>
			    ...
			    <td><a href="${delete}" onclick="return Delete_Dialog()">削除</a> | <a href="${update}">編集</a></td>
			 	<td><c:out value="${goods.getGoods_id()}" /></td>
			 	<td><c:out value="${goods.getGoods_name()}" /></td>
			 	<td><c:out value="${goods.getGoods_price()}" /></td>
			 	<td><c:out value="${goods.getGoods_cost()}" /></td>
			 </tr>
   	 </c:forEach>
